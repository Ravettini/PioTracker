...
export class AnalyticsService {
  ...
  private async getDataFromLocalDatabase(indicadorId: string, periodoDesde?: string, periodoHasta?: string): Promise<any[]> {
    this.logger.log(`📊 Buscando datos en base de datos local para indicador: ${indicadorId}`);
    ...
    this.logger.log(`📊 Encontradas ${cargas.length} cargas en base de datos local`);

    const datos = cargas.map(c => ({
      periodo: c.periodo,
      mes: c.periodo, // Usar período como mes si no hay campo mes específico
      valor: c.valor,
      meta: c.meta,
      unidad: c.unidad,
      fuente: c.fuente,
      responsable: c.responsableNombre,
      responsableEmail: c.responsableEmail,
